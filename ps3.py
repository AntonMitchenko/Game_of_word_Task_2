...
def get_word_score(word, n):
    ...
    score = score_for_letters * max(((HAND_SIZE * worddlen) - (3 * (n - worddlen))), 1)
    return score
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while calculate_handlen(hand) > 0:
        ...
        # Otherwise (the input is not two exclamation points), If the word is valid:
        elif is_valid_word(word, hand, word_list):
            score = get_word_score(word, calculate_handlen(hand))
            total_score += score
            # Tell the user how many points the word earned and the updated total score
            print(f'"{word}" earned {score}. Total: {total_score} points\n')
...
def substitute_hand(hand, letter):
    ...
    set_letters = set(all_letters) - set(hand.keys())
    set_letters = ''.join(set_letters)
    random_letter = random.choice(set_letters)
...
def get_total_number_of_hands():
    total_number_of_hands = input('Please, write total number of hands: ')
    try:
        total_number_of_hands = int(total_number_of_hands)
        if total_number_of_hands < 1:
            print('Please, write correct number')
            return get_total_number_of_hands()
        else:
            return total_number_of_hands
    except:
        print('Please, write correct number')
        return get_total_number_of_hands()
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    while i_hand < number_of_hands:
        init_hand = deal_hand(HAND_SIZE)
        curr_hand = init_hand.copy()
